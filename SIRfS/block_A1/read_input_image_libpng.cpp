...
Matrix2D<double> read_png_file()
{
    ...
    {
        cout<<"[Reading input image] Could not define read structure pointer!"<<endl;
        exit(0);
    }
    ...
    {
        cout<<"[Reading input image] Could not ldefine info structure pointer!"<<endl;
    ...
    {
        cout<<"[Reading input image] Could not save program's state!"<<endl;
    ...
    //the first 8 bytes were checked, so tell png_struct_pointer that these bytes cannot be found at the known pointer location
    ...
    //convert gray image to RGB as the grayscaling method uses the classic average, wheeras the input one might have used another
    ...
    //check the remainder of the image for correctness (remaining chunks)
