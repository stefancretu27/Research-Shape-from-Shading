...
void ColorGaussian::initializeColorLaboratoryGaussianData()
{
    ...
    ldFileReader.readVector("prior/lights/color/laboratory/gaussian/mu_exp17.txt", this->mu, 27, 17);

    //read data to matrix
    DataFile<double> dFileReader;
    dFileReader.readMatrix2D("prior/lights/color/laboratory/gaussian/Sigma_exp17.txt", this->Sigma, 27, 27, 17);
}
...
void ColorGaussian::initializeColorNaturalGaussianData()
{
    ...
    //read data to vector
    DataFile<long double> ldFileReader;
    ldFileReader.readVector("prior/lights/color/natural/gaussian/mu20.txt", this->mu, 27, 20);

    //read data to matrix
    DataFile<double> dFileReader;
    dFileReader.readMatrix2D("prior/lights/color/natural/gaussian/sigma20.txt", this->Sigma, 27, 27, 20);
}
