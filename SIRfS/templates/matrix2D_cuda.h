...
#define CUDA_MATRIX_2D 

#ifdef CUDA_MATRIX_2D
...
template<class Type> cudaError_t operator_add(Type* device_A, Type* device_B, int grid_dim, int block_dim, int matrix_dim);
template<class Type> cudaError_t operator_substract(Type* device_A, Type* device_B, int grid_dim, int block_dim, int matrix_dim);
...
public:
    //constructors
    __host__ __device__ Matrix2D(){this->rows = 0; this->cols = 0; this->container = nullptr;};
    //overloaded constructors
    __host__ __device__ Matrix2D(int new_rows, int new_cols);
    __host__ __device__ Matrix2D(int new_rows, int new_cols, Type value);
    //copy constructor
    __host__ __device__ Matrix2D(const Matrix2D<Type>& input_matrix);
    //destructor
    __host__ __device__ ~Matrix2D()
    {
        //delete the array
    ...
    //indexes operations
    __host__ __device__ inline unsigned int getLinearIndex(int i, int j)
    {
        return i*this->cols + j;
    };
    __host__ __device__ void get2DIndecesFromLinearIndex(int index, int &rows, int &cols)
    {
    ...
    //getters
    __host__ __device__ inline int getRows() const              //inline tells compiler to replace function with its definition => faster execution as there is less linking
    {
        return this->rows;
    };
    __host__ __device__ inline int getCols() const
    {
        return this->cols;
    };
    __host__ __device__ inline int getDim() const
    {
        return this->rows*this->cols;
    };
    __host__ __device__ inline Type* getContainerPointer()
    {
        return this->container;
    };
    __host__ __device__ inline Type getMatrixValue(int i, int j) const
    {
        return this->container[i*this->cols + j];
    };
    __host__ __device__ inline Type getMatrixValue(unsigned linearindex) const
    {
        return this->container[linearindex];
    };

    //setters
    __host__ __device__ inline void setRows(int new_rows)
    {
        this->rows = new_rows;
    };
    __host__ __device__ inline void setCols(int new_cols)
    {
        this->cols = new_cols;
    };
    __host__ __device__ inline void setDims(int new_rows, int new_cols)
    {
        this->cols = new_cols;
        this->rows = new_rows;
    };
    __host__ __device__ inline void setMatrixValue(int i, int j, Type value)
    {
        this->container[i*this->cols +j] = value;
    };
    __host__ __device__ inline void setMatrixValue(unsigned int linearindex, Type value)
    {
        this->container[linearindex] = value;
    };
    __host__ __device__ void setMatrix2D(Type* data, int new_rows,  int new_cols, bool transp);

    //operators overloading
    __host__ __device__ inline Type& operator() (int rowsIdx, int colsIdx) const
    {
        return this->container[rowsIdx * this->cols + colsIdx];
    };
    __host__ __device__ bool operator==(const Matrix2D<Type>& operand_matrix);
    __host__ __device__ Matrix2D& operator=(const Matrix2D<Type>& operand_matrix);
    __host__ __device__ Matrix2D& operator+(Matrix2D<Type>& operand_matrix);
    __host__ __device__ Matrix2D& operator-(Matrix2D<Type>& operand_matrix);
    __host__ __device__ Matrix2D& multiply_input_matrices(Matrix2D<Type>& input_A, Matrix2D<Type>& input_B);
    
    //helper: separate function from constructors, used data copying
    __host__ __device__ void initializeMatrixValues(Type value);

    //convolution: apply kernel to the input matrix and store the conv result in the caller matrix
    __host__ __device__ void conv2DFull(Matrix2D<Type>& kernel, Matrix2D<Type>& input);
    __host__ __device__ void conv2DValid(Matrix2D<Type>& kernel,  Matrix2D<Type>& input);
    __host__ __device__ void conv2DSame(Matrix2D<Type>& kernel, Matrix2D<Type>& input);
    ...
    */
    ...
    __host__ __device__ void insertNaNValues(Matrix2D<bool>& mask);
    //creates a mask by comparing the caller's values to the given treshold
    __host__ __device__ void compareValuesToTreshold(Matrix2D<bool>& result, Type treshold, Comparison comp);
//CHANGED
    __host__ __device__ void negateMatrixMask(const Matrix2D<bool>& input_mask);
    //returns the # of non-zero elements
    __host__ __device__ int logicalAnd(Matrix2D<bool>& result, const Matrix2D<Type>& input);
    //only the number of columns has to be equal to size of vector
    __host__ __device__ void compareMatrixColumnsToVector(Matrix2D<bool>& result, const std::vector<Type>& input, Comparison comp);
    __host__ __device__ void applyVectorMask(const Matrix2D<Type>& input, const std::vector<bool> mask);
    __host__ __device__ void applyDoubleVectorMask(const Matrix2D<Type>& input, const std::vector<bool> mask1, const std::vector<bool> mask2);
    __host__ __device__ void applyMatrixMask(const Matrix2D<Type>& input, const Matrix2D<bool> mask);
    __host__ __device__ void sortLines();

    /*
    *basic math-operations
    */
    __host__ __device__ void getAbsoluteValuesMatrix(const Matrix2D<Type>& input);
    __host__ __device__ void logNatMatrix(const Matrix2D<Type>& input);
    __host__ __device__ void findFirstNonEqualElement(int &x_first, int &y_first, Type value);
    __host__ __device__ void findLastNonEqualElement(int &x_last, int &y_last, Type value);
    //for the following 2 functions, the output is a vector
    __host__ __device__ void anyNonZero(std::vector<int>& result, int direction);
    __host__ __device__ void anyGreater(std::vector<int>& result, int direction, int treshold);
    __host__ __device__ bool checkNonZero();
    //the equivalent of  'find' Matlab function, but stores indeces in a matrix with 2 columns
    __host__ __device__ void mFindIndeces(Matrix2D<int>& result, Type value, Comparison cmp);
    //same as above but the output is a matrix with 3 column, as the 3rd stores also the values
    __host__ __device__ void mFindIndecesAndValues(Matrix2D<Type>& result, Type value, Comparison cmp);
    //same as mFindIndeces, but stores output indeces in 2 separate vectors
    __host__ __device__ void vFindIndeces(std::vector<int>& x, std::vector<int>&  y, Type value, Comparison cmp);
    //same as above, but stores the values in a 3rd vector
    __host__ __device__ void vFindIndecesAndValues(std::vector<int>& x, std::vector<int>&  y, std::vector<Type>&  val, Type value, Comparison cmp);
    //per matrix element operation
    __host__ __device__ void elementsOperation(Matrix2D<Type>& result, Type value, Operation op);
    __host__ __device__ void allNonZero(std::vector<bool>& result, int direction);
    __host__ __device__ int countValuesDifferentFromInput(Type value);
    /*
    *matrix conversions
    */
    __host__ __device__ void copyMatrixColumnToVector(std::vector<Type> &result, int col);
    __host__ __device__ void copyMatrixRowToVector(std::vector<Type> &result, int row);
    __host__ __device__ void reshapeToVector(std::vector<Type>& dest);
//CHANGED
    __host__ __device__ void getSubMatrix(const Matrix2D<Type>& input, int x_first, int x_last, int y_first, int y_last);
//CHANGED
    __host__ __device__ void reverseMatrix(const Matrix2D<Type>& input);
    __host__ __device__ void getTranspose(Matrix2D<Type>** result);
    //same as above, but result is stored in the caller (caller data is lost)
    __host__ __device__ void TransposeMatrix();
    
    //for debug purposes
    __host__ __device__ void printMatrixValues()
    {
        for(int i = 0; i < 10/*this->rows*/; i++)
        {
...
template <class Type>
__host__ __device__ Matrix2D<Type>::Matrix2D(int new_rows, int new_cols):rows(new_rows), cols(new_cols)
{
...
template <class Type>
__host__ __device__ Matrix2D<Type>::Matrix2D( int new_rows,  int new_cols, Type value):rows(new_rows), cols(new_cols)
{
    if(this->rows > 0 && this->cols > 0)
    {
        //allocate memory
...
template <class Type>
__host__ __device__ Matrix2D<Type>::Matrix2D(const Matrix2D<Type>& input_matrix):rows(input_matrix.getRows()), cols(input_matrix.getCols())
{
    if(this->rows > 0 && this->cols > 0)
    {
        //allocate memory
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::setMatrix2D(Type* data, int new_rows,  int new_cols, bool transp)
{
    this->rows = new_rows;
    this->cols = new_cols;
...
//operators overloading
template <class Type>
__host__ __device__ bool Matrix2D<Type>::operator==(const Matrix2D<Type>& operand_matrix)
{
    bool k = true;
...
template <class Type>
__host__ __device__ Matrix2D<Type>& Matrix2D<Type>::operator=(const Matrix2D<Type>& operand_matrix)
{
    //get new dimensions
...
template <class Type>
__host__ __device__ Matrix2D<Type>& Matrix2D<Type>::operator+(Matrix2D<Type>& operand_matrix)
{
...
/*
#ifdef CUDA_MATRIX_2D
	Type *device_A, *device_B;
	//alocate meory on GPU
	cudaMalloc(&device_A, sizeof(Type) * this->getDim());
	cudaMalloc(&device_B, sizeof(Type) * this->getDim());
	
	//copy elements in GPU RAM. Parameters: destination, source, size, flag
	cudaMemcpy(device_A, this->getContainerPointer(), sizeof(Type) * this->getDim(), cudaMemcpyHostToDevice);
	cudaMemcpy(device_B, operand_matrix.getContainerPointer(), sizeof(Type) * this->getDim(), cudaMemcpyHostToDevice);
	
	int dimGrid = (this->getDim() + THREADS_PER_ONE_DIM_BLOCK -1)/THREADS_PER_ONE_DIM_BLOCK;
	//call kernel launcher
	operator_add(device_A, device_B, dimGrid, THREADS_PER_ONE_DIM_BLOCK, this->getDim());
	//copy the results from GPU RAM into CPU RAM
	cudaMemcpy(this->getContainerPointer(), device_A, sizeof(Type) * this->getDim(), cudaMemcpyDeviceToHost);
...
/*
	...
	int dimGrid = (this->getDim() + THREADS_PER_ONE_DIM_BLOCK -1)/THREADS_PER_ONE_DIM_BLOCK;
	//call kernel launcher
	operator_substract(device_A, device_B, dimGrid, THREADS_PER_ONE_DIM_BLOCK , this->getDim());
	//copy the results from GPU RAM into CPU RAM
	cudaMemcpy(this->getContainerPointer(), device_A, sizeof(Type) * this->getDim(), cudaMemcpyDeviceToHost);
...
template <class Type>
__host__ __device__ Matrix2D<Type>& Matrix2D<Type>::multiply_input_matrices(Matrix2D<Type>& input_A, Matrix2D<Type>& input_B)
{
	...
	//alocate meory on GPU
	cudaMalloc(&device_A, sizeof(Type) * input_A.getDim());
	cudaMalloc(&device_B, sizeof(Type) * input_B.getDim());
	cudaMalloc(&device_result, sizeof(Type) * this->getDim());
...
*/
template <class Type>
__host__ __device__ void Matrix2D<Type>::initializeMatrixValues(Type value)
{
...
*/
template <class Type>
__host__ __device__ void Matrix2D<Type>::conv2DFull(Matrix2D<Type>& kernel, Matrix2D<Type>& input)
{
#ifdef CUDA_MATRIX_2D
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::conv2DValid(Matrix2D<Type>& kernel,  Matrix2D<Type>& input)
{
#ifdef CUDA_MATRIX_2D
	Type *device_output, *device_input, *device_kernel;
	//alocate memory on GPU
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::conv2DSame(Matrix2D<Type>& kernel, Matrix2D<Type>& input)
{
#ifdef CUDA_MATRIX_2D
	Type *device_output, *device_input, *device_kernel;
	//alocate memory on GPU
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::insertNaNValues(Matrix2D<bool>& mask)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::compareValuesToTreshold(Matrix2D<bool>& result, Type treshold, Comparison comp)
{
    switch(comp)
    {
        case 0:
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::negateMatrixMask(const Matrix2D<bool>& input_mask)
{
...
template <class Type>
__host__ __device__ int Matrix2D<Type>::logicalAnd(Matrix2D<bool>& result, const Matrix2D<Type>& input)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::compareMatrixColumnsToVector(Matrix2D<bool>& result, const std::vector<Type>& input, Comparison comp)
{
    int i , j;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::applyVectorMask(const Matrix2D<Type>& input, const std::vector<bool> mask)
{
    int t_idx = 0;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::applyDoubleVectorMask(const Matrix2D<Type>& input, const std::vector<bool> mask1, const std::vector<bool> mask2)
{
    int idx = 0, idy = 0;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::applyMatrixMask(const Matrix2D<Type>& input, const Matrix2D<bool> mask)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::sortLines()
{
    Type aux;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::getAbsoluteValuesMatrix(const Matrix2D<Type>& input)
{
    for(unsigned int idx = 0; idx < input.getDim(); idx++)
    {
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::logNatMatrix(const Matrix2D<Type>& input)
{
    for(unsigned int idx = 0; idx < input.getDim(); idx++)
    {
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::findFirstNonEqualElement(int &x_first, int &y_first, Type value)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::findLastNonEqualElement(int &x_first, int &y_first, Type value)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::anyNonZero(std::vector<int>& result, int direction)
{
    if(direction == 1)
    {
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::anyGreater(std::vector<int>& result, int direction, int treshold)
{
...
template <class Type>
__host__ __device__ bool Matrix2D<Type>::checkNonZero()
{
    for(unsigned int idx = 0; idx < this->getDim(); idx++)
    {
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::mFindIndeces(Matrix2D<int>& result, Type value, Comparison cmp)
{
    int res_i = 0, i, j;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::mFindIndecesAndValues(Matrix2D<Type>& result, Type value, Comparison cmp)
{
    int res_i = 0, i, j;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::vFindIndeces(std::vector<int>& x, std::vector<int>&  y, Type value, Comparison cmp)
{
    int res_i = 0, i, j;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::vFindIndecesAndValues(std::vector<int>& x, std::vector<int>&  y, std::vector<Type>&  val, Type value, Comparison cmp)
{
    int res_i = 0, i, j;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::elementsOperation(Matrix2D<Type>& result, Type value, Operation op)
{
    switch(op)
    {
    case 0:
...
//Returns 1 if a row/column has at least a non-zero value, else it returns 0 for that row/column. The output is a vector. Direction: 1 = check on columns; 2 = check on rows
template <class Type>
__host__ __device__ void Matrix2D<Type>::allNonZero(std::vector<bool>& result, int direction)
{
    if(direction == 1)
    {
        for(int j = 0; j < this->getCols(); j++)
...
template <class Type>
__host__ __device__ int Matrix2D<Type>::countValuesDifferentFromInput(Type value)
{
    int counter = 0;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::copyMatrixColumnToVector(std::vector<Type> &result, int col)
{
    for(int i = 0; i < this->getRows(); i++)
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::copyMatrixRowToVector(std::vector<Type> &result, int row)
{
    for(int j = 0; j < this->getCols(); j++)
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::reshapeToVector(std::vector<Type>& dest)
{
    for(int j = 0; j < this->getCols(); j++)
        for(int i = 0; i < this->getRows(); i++)
        {
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::getSubMatrix(const Matrix2D<Type>& input, int x_first, int x_last, int y_first, int y_last)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::reverseMatrix(const Matrix2D<Type>& input)
{
    Type temp;
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::getTranspose(Matrix2D<Type>** output)
{
...
template <class Type>
__host__ __device__ void Matrix2D<Type>::TransposeMatrix()
{
    Type temp;
